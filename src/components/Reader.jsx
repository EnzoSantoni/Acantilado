...
export default function Reader({currentChapter, onOpenChapter}) {
    ...
    return (
        <section id='lector' className={`bg-papel py-20 md:py-28 font-display transition-colors duration-500 lectura:bg-noche ${readingMode ? 'reading-mode' : ''}`}>
            <div className='mx-auto max-w-7xl px-5 md:px-10'>
                <div className='mb-9 flex items-center justify-between border-b border-tinta/45 pb-5 lectura:border-papel/20'>
                    <p className="text-[18px] leading-4 font-bold uppercase tracking-[.2em] text-bordo">Sala de lectura</p>
                    <button type="button" className={`inline-flex items-center gap-2 rounded-full border border-bordo px-4 py-2 leading-4 font-bold uppercase tracking-[.12em] text-bordo transition hover:bg-bordo hover:text-papel lectura:border-tinta lectura:bg-tinta lectura:text-papel ${foco}`} onClick={handleReadingMode} aria-pressed={readingMode}>
                        <Focus className="size-4" />
                        <span className="text-[18px] font-bold">Modo lectura</span>
                    </button>
                </div>
                <div className="mx-auto max-w-4xl rounded-xs border border-tinta/25 bg-papel px-6 py-12 transition-all duration-300 md:px-16 md:py-16 lectura:max-w-190 lectura:shadow-[0_0_0_1px_rgba(244,235,220,.3),0_18px_60px_rgba(244,235,220,.2),0_0_90px_rgba(244,235,220,.12)]">
